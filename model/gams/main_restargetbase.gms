...
$if not set scenario $set scenario restargetbase
*base

* set ntc_adjust
$if not set ntc_adjust $set ntc_adjust 2

*alternative renewable share target (distributed to NECP targets if scenario separate)
$if not set newtarget $set newtarget
...
$set reportfile "%reportdir%%scenario%%modeltype%%runno%%suffix%%ntc_adjust%.gdx"
