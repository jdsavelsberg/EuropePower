...
$if not set scenario $set scenario restargetcombined
*base

* set ntc_adjust
$if not set ntc_adjust $set ntc_adjust 1.5

*alternative renewable share target (distributed to NECP targets if scenario separate)
$if not set newtarget $set newtarget 0.8


*Tyndp settings if used
$if not set tyndpscenario $setglobal tyndpscenario "GlobalAmbition"
$if not set climateyear $setglobal climateyear "1982"
...
$if not set res_target_base $set res_target_base on
$if not set module_carbon $set module_carbon on
$if not set module_tyndp $set module_tyndp on
...
$if not set baseData $setglobal baseData "data_EU_2017_all"
...
$set reportfile "%reportdir%%scenario%%modeltype%%runno%%suffix%%newtarget%%ntc_adjust%.gdx"
...
$if not set linelosses $setglobal linelosses 0
*used to be 0.03
...
$if not %ntc_adjust%==""  ntc(c,cc,t)= ntc(c,cc,t)*%ntc_adjust%  
