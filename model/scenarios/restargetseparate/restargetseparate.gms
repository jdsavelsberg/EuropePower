...
*$setglobal combined_reduction_target 0.395
*$setglobal targetshareets1 0.352
...
min_sh_renewables(q) = sum(c,RES_targets(c,q))/100
;

$gdxin ../scenarios/restargetbase/results/restargetbaseQCP%suffix%%ntc_adjust%.gdx
$load dem_res_base=DEM.L

$if %newtarget%==0.8 res_separate_multiplier = %newtarget% / (sum(q,min_sh_renewables(q)*sum(c$(sum(tech$map_q(tech,c,q), 1) > 0),dem_res_base(c))) / sum(q,sum(c$(sum(tech$map_q(tech,c,q), 1) > 0), dem_res_base(c))));
$if %newtarget%==0.8 min_sh_renewables(q) = res_separate_multiplier *  sum(c,RES_targets(c,q)/100);
